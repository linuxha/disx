...
//  { 'q',  &DisScrn::do_cmd_q   }, // exit program
    ...
    { 'B',  &DisScrn::do_cmd_B   }, // disassemble as decimal byte
    ...
    { 'E',  &DisScrn::do_cmd_e   }, // disassemble as word offset table entry
    { 0x05, &DisScrn::do_cmd_E   }, // disassemble as byte offset table entry
...
//  { "g",      &DisScrn::do_cmd_go     }, // "g"      - go to address
    ...
    { "hex",    &DisScrn::do_cmd_hex    }, // "hex"    - display code data in hex
    { "octal",  &DisScrn::do_cmd_octal  }, // "octal"  - display code data in octal
    {  NULL,    NULL  }
};
...
#if 0
    printf("_start = %.4zX:%d\n", scrn._start.addr, scrn._start.line);
    printf("_top   = %.4zX:%d\n", scrn._top.addr, scrn._top.line);
    printf("_sel   = %.4zX:%d\n", scrn._sel.addr, scrn._sel.line);
    printf("_end   = %.4zX:%d\n", scrn._end.addr, scrn._end.line);
    printf("_sel_row  = %3d\n", scrn._sel_row);
//  printf("curs_row  = %3d\n", screen_row(_sel));
//  printf("end_row   = %3d\n", screen_row(_end));
    printf("max_row() = %3d\n", scrn.max_row());
...
void DisScrn::debug_info()
{
    ...
    wmove(_win, row++, col);
    wprintw(_win, " _start = %.4zX:%d ", _start.addr, _start.line);

    wmove(_win, row++, col);
    wprintw(_win, " _top   = %.4zX:%d ", _top.addr, _top.line);

    wmove(_win, row++, col);
    wprintw(_win, " _sel   = %.4zX:%d ", _sel.addr, _sel.line);

    wmove(_win, row++, col);
    wprintw(_win, " _end   = %.4zX:%d ", _end.addr, _end.line);
...
void DisScrn::error(const char *s)
{
    ...
    if (s && s[0]) {
        ...
        // flush any other keyboard input that may be pending
        flushinp();
    } else {
...
// =====================================================
void DisScrn:: do_go_addr(addr_t addr)
{
...
// =====================================================
void DisScrn::do_cmd_hex(char *p)
{
    (void) p; // unused parameter
    curCpu->set_octal(false);
    print_screen();
}


// =====================================================
void DisScrn::do_cmd_octal(char *p)
{
    (void) p; // unused parameter
    curCpu->set_octal(true);
    print_screen();
}
...
void DisScrn::do_cmd_label(char *p)
{
    ...
    // don't allow ":" at end of label name
...
// =====================================================
// change this line to type = mDecB
void DisScrn::do_cmd_B()
{
    // first get the line length
    int len = _count;
    ...
    rom.set_instr(_sel.addr, len, mDecB);
...
// =====================================================
// change this line to type = mOfsW
void DisScrn::do_cmd_e()
{
    // set up for repeat
    _rpt_key = 'e';
    _rpt_count = 0;
...
//(void) addr;
    ...
    rom.set_instr(_sel.addr, 2, mOfsW);

#if 0
printf(" *** addr=%.8zX len=%d attr=%.2X type=%.2X mOfsW=%.2X ***\r\n",
 _sel.addr, rom.get_len(_sel.addr), rom.get_attr(_sel.addr), rom.get_type(_sel.addr), mOfsW); fflush(stdout);
#endif
...
// =====================================================
// change this line to type = mOfsB
void DisScrn::do_cmd_E()
{
    // set up for repeat
    _rpt_key = 'E';
...
//(void) addr;
    ...
    // set the instruction type
    rom.set_instr(_sel.addr, 1, mOfsB);
...
void DisScrn::do_cmd_cln()
{
    ...
    if (label) {
        sprintf(s, "label addr = %.4zX", label);
        error(s);
    }
#endif
    ...
    if (label) {
            ...
            if (rom.get_type(ad) != mData && !rom.test_attr(ad, ATTR_CONT)) {
                ...
                if ((lfref & (REFFLAG|CODEREF)) && (refaddr == label)) {
                    if (defCpu->_radix & RAD_OCT) {
                        sprintf(s, "Found label %.6zo at %.6zo", label / scale,
                                                                 ad / scale);
                    } else {
                        sprintf(s, "Found label %.4zX at %.4zX", label / scale,
                                                                 ad / scale);
                    }
                    error(s);
                    found = true;
        ...
        if (!found) {
            ...
            if (defCpu->_radix & RAD_OCT) {
                sprintf(s, "Deleted label at %.6zo", label / scale);
            } else {
                sprintf(s, "Deleted label at %.4zX", label / scale);
            }
            error(s);
...
void DisScrn::do_cmd_prv()
{
    ...
    if (_sel.addr > rom._base) {
        _sel.prev_line();
        while (_sel.addr > rom._base
            && (!rom.test_attr(_sel.get_line_addr(), ATTR_LMASK)
                   // skip pre-LF lines
                || (_sel.line == 0 && rom.test_attr(_sel.addr, ATTR_LF0))
                   // skip post-LF lines
                || (_sel.line != 0 && rom.test_attr(_sel.addr, ATTR_LF1)
                                   && _sel.addr == _sel.get_line_addr())
              )) {
            _sel.prev_line();
        }
        recenter(true);
